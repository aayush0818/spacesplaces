...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-5 font-medium transition-all duration-300",
    ...
    >
      {children}
      <ChevronDown className="h-5 w-5 shrink-0 text-muted-foreground transition-all duration-300" />
    </AccordionPrimitive.Trigger>
